...
\begin{enumerate}
  \item[] \textbf{Question:} You are given the root of a binary tree root. Invert the binary tree and return its root. 
    ...
    \begin{enumerate}
      ...
      \item[-] In explore function, first check if the current node is not None, if not None, add current node into list of visited nodes. If None, return
      \item[-] Next create a temp node, and swap the left and right children. Then explore left node, and explore right node
