...
Can you write a solution that runs in O(log(m * n)) time?
    \begin{enumerate}
      ...
      \item[-] \textbf{Edge Case:} When checking if the value is greater than the first in the row, must also check if the value is smaller than the last value in that row. This means that the value is \textit{in} that row.
